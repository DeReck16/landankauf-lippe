...
export default async function VorgaengePage(props: PageProps<"/admin/vorgaenge">) {
  ...
  for (const x of vorgaenge) {
    if (!x.v) continue;
    if (M.aktiveFreigabe(x.v) && !x.v.mails.some((m) => m.zweck === "freigabe" && m.ok)) faellig.push({ text: `Freigabe-Mitteilungen senden: ${name(x.v.angebotId)} ↔ ${name(x.v.gesuchId)}`, href: `/admin/vorgang/${x.key}#entwuerfe`, tipp: "Die Kontaktdaten sind freigegeben, aber noch keine Mitteilung ist rausgegangen" });
