...
import { haText, ladeBoerse, provisionOderStandard } from "@/lib/boerse";
...
export default async function Page(props: PageProps<"/flaechenboerse/[code]">) {
  ...
  return (
    <>
      <PageHero eyebrow={`Flächenbörse · Angebot ${a.code}`} title={`${a.typ || "Fläche"} zu kaufen, ${haText(a.groesseHa)}`} subtitle={`${a.lage} — anonym angeboten. Namen, Kontaktdaten und Flurstücke erhalten Sie nach Vertragsabschluss und Zustimmung des Eigentümers.`} />
      <section className="section">
        <div className="container-page grid gap-10 lg:grid-cols-[1fr_1.2fr]">
          <div>
            ...
            <div className="mt-6 text-[color:var(--color-ink-soft)] space-y-3">
              <p>
                <strong>So geht es weiter:</strong> Sie melden hier unverbindlich Ihr Interesse an. Wir schicken Ihnen einen persönlichen Zugang, dort schließen Sie online einen kurzen Nachweisvertrag. Stimmt der Eigentümer dem Kontakt zu, sehen Sie Namen, Kontaktdaten und Flurstücke — den Kaufvertrag schließen Sie beim Notar.
              </p>
              <p>
                <strong>Provision nur bei Erfolg:</strong> {provision}. Kommt kein Kauf zustande, zahlen Sie nichts.
              </p>
              <p>
